...
作为系列初代模型，Qwen-VL基于Qwen-7B语言模型，首次实现视觉-语言多模态融合。其核心目标是构建通用视觉理解能力，并填补开源LVLM在**细粒度视觉定位与文本阅读**方面的空白。

#### 核心技术创新
- **轻量化架构设计**：采用“视觉编码器+位置感知适配器+LLM”三段式结构，视觉编码器基于ViT-bigG（1.9B参数）初始化，通过**单层交叉注意力适配器**将图像特征压缩至**固定长度**256 tokens（解决长序列效率问题），并引入2D绝对位置编码保留细粒度空间信息，总参数仅9.6B（LLM占7.7B），兼顾性能与效率。
...
- **三段式训练 pipeline**：
  ...
  > 多段式训练是大型模型（尤其是多模态模型）应对数据异质性、任务复杂性和资源约束的**工程优化策略**，通过分阶段学习降低训练难度、提升稳定性和效率。对于Qwen-VL这类多模态大模型，多段式训练几乎是**必要选择**；而对于简单单模态任务或小规模数据，端到端训练可能更直接。实际应用中需根据数据特征、任务需求和资源条件综合决策。
...
- **局限性**：仅支持静态图像，无视频理解能力；图像分辨率固定（448×448），动态分辨率适配差；上下文长度有限（2048 tokens），长文档/多图像处理能力弱。
  ...
  <span class="image main">
  <img class="main img-in-blog" style="max-width: 50%" src="./blogs/33_Qwen_VL_Series_Papers_Reading_Report/Qwen-VL_Performance.webp" alt="Qwen-VL Performance" />
...
- **原生动态分辨率（Naive Dynamic Resolution）**：
  ...
  - 推理时按GPU显存限制动态打包不同分辨率图像，通过MLP层压缩2×2相邻图像块，平衡精度与效率。
