...
**为什么 Transformer 特别偏爱 GeLU？**

Transformer 的架构特点决定了它对激活函数的特殊要求，而 GeLU 恰好是“量身定做”的：

| Transformer 的需求                                           | GeLU 的解决方案                                            |
| ------------------------------------------------------------ | ---------------------------------------------------------- |
| **深度堆叠**：Encoder/Decoder 层数极深，需要良好的梯度传导。 | **处处可导**：平滑的曲线确保了深层梯度稳定回流。           |
| **概率建模**：Attention 机制本质是加权求和，需要非线性的概率解释。 | **概率门控**：非线性的平滑过渡更符合注意力权重的概率特性。 |
| **归一化输入**：LayerNorm 使输入趋于 `N(0,1)`，需要激活函数对此敏感。 | **高斯平滑**：对 0 附近的小值有细腻的处理，保留更多信息。  |
