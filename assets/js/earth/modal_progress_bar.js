...
function loadImageWithProgress(imageSrc) {
    ...
    modalProgressInterval = setInterval(() => {
        if (modalImageLoadProgress < 90 && !isImageLoaded) {
            modalImageLoadProgress += Math.random() * 15 + 5; // 5-20%随机增量
            if (modalImageLoadProgress > 90) modalImageLoadProgress = 90;

            // 更新进度条
            modalProgressBar.style.width = modalImageLoadProgress + '%';
            modalProgressText.textContent = Math.round(modalImageLoadProgress) + '%';
            ...
            // 更新占位符文本
            placeholderText.textContent = `加载中... ${Math.round(modalImageLoadProgress)}%`;
        }
    }, 200);
